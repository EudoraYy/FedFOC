...
class TensorDataset():
    ...
    def subsample(self, total_size, equal_classes_ratio=True):  # equal_classes_ratio=True

        idxs = torch.arange(self.features.shape[0])
        classes = list(set(self.labels.numpy()))
        # gcd = math.gcd(classes[0], classes[1])
        if equal_classes_ratio:  # equal_classes如果为True，表示需要按照原数据集中某类别样本占所有样本的比例进行采样
            sample_idx = []
            for c in classes:
                class_ratio = (self.labels == c).sum() / len(self.labels)  # 计算原数据集中的类别所占比例
                n_samp = int(total_size * class_ratio)
                # n_samp = (self.labels == c).sum() / gcd
...
class KeopsRoutine():
    ...
    def routine(self, α, x, β, y, class_xy=None, class_yx=None, class_xx=None, class_yy=None, mask_diagonal=False,
                **kwargs):
...
class SamplesLossTensorized(CostFunction):
    ...
    def distance_with_labels(self, x_vals, y_vals, x_labels, y_labels,
                             ...
                             mask_diagonal=False):
        class_vals_xy = self.label_distances(x_vals, y_vals, x_labels, y_labels, gaussian=gaussian_class_distance)
        # print("标签之间的距离为：{}".format(class_vals_xy[0]))
        d_xy = get_class_matrix(x_labels, y_labels, *class_vals_xy).type_as(x_vals)
        # print("d_xy is:{}".format(d_xy))
        # print("x_labels is: {};\ny_labels is:{}".format(x_labels, y_labels))
