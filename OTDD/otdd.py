...
def get_class_matrix(x_labels, y_labels, class_distances, class_x_dict=None, class_y_dict=None):
    # print("class_distances is:{}\n".format(class_distances))
    ...
    dz = np.zeros((len(x_labels), len(y_labels)))

    for i in range(len(x_labels)):
        for j in range(len(y_labels)):
            # 将标签与标签的距离映射到每个样本到其他样本标签的距离
            dz[i][j] = class_distances[class_x_dict[x_labels[i].item()]][class_y_dict[y_labels[j].item()]]
    dz = torch.from_numpy(dz)
    # print("样本与样本标签之间的距离为：{};\n数组形状为：{}".format(dz, dz.shape))
...
class CostFunction():
    ...
    def label_distances(self, x_vals, y_vals, x_labels, y_labels, max_iter=None, gaussian=False):
        ...
        x_labels = torch.tensor(x_labels, device=device)
        y_labels = torch.tensor(y_labels, device=device)
        ...
        distances = torch.zeros((len(class_x), len(class_y)), device=device)
        ...
        if gaussian:
            ...
            for c1 in class_x:
                ...
                mu_xs.append(sample_x.mean(dim=0, keepdim=True))  # 计算均值
                cov_xs.append(torch.cov(sample_x.T))  # 计算协方差矩阵

            for c2 in class_y:
                sample_y = y_vals[y_labels == c2]
                mu_ys.append(sample_y.mean(dim=0, keepdim=True))
                cov_ys.append(torch.cov(sample_y.T))
        ...
        return distances.cpu().numpy(), class_x_dict, class_y_dict
